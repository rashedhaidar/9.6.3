import React, { useState, useEffect, useCallback, useMemo } from 'react';
    ...
    export function ActivityForm({
      ...
      const handleSubmit = useCallback(async (e: React.FormEvent) => {
        ...
        setGoalId('');
      }, [title, description, targetCount, selectedDays, allowSunday, reminderTime, domainId, onSubmit, selectedDay, weekDates, initialDomainId, goalId]);
      ...
      const getGoalsByDomain = (domainId: string) => {
        const saved = localStorage.getItem('goals');
        const goals = saved ? JSON.parse(saved) : [];
        return goals.filter((goal: any) => goal.domainId === domainId);
      };

      const domainGoals = useMemo(() => getGoalsByDomain(domainId), [domainId]);
